...
class InsightsViewModel(
    ...
    private fun loadStats() {
        viewModelScope.launch {
            ...
            val allTasks = taskRepository.observeAll()
            val taskList = allTasks.first()
            val weekTasks = taskList.filter { it.dueDateEpochDay in start..end && !it.isDeleted }
            val taskTotal = weekTasks.size
            _stats.value = InsightStats(tasksCompleted, taskTotal, spend, diaryCount)
        }
    }
...
data class InsightStats(
    ...
    val completionPercent: Int get() = if (tasksTotal == 0) 0 else (tasksCompleted * 100 / tasksTotal).coerceIn(0, 100)
}

@Composable
fun InsightsScreen() {
    val locator = LocalServiceLocator.current
    val viewModel: InsightsViewModel = viewModel(factory = LambdaViewModelFactory {
    ...
    LazyColumn(
        ...
        contentPadding = PaddingValues(horizontal = LifeOSSpacing.screenPadding, top = 6.dp, bottom = 110.dp),
            ...
            LifeOSCard(Modifier.fillMaxWidth(), tint = MaterialTheme.colorScheme.surface) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Column(Modifier.weight(1f)) {
                        ...
                        Text("A simple view of this week's activity", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                        Text("${stats.completionPercent}%", style = MaterialTheme.typography.displaySmall, color = LifeOSPrimary, modifier = Modifier.padding(top = 8.dp))
                        Text("task completion", style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                    CircularProgressIndicator(progress = { stats.completionPercent / 100f }, modifier = Modifier.height(92.dp), strokeWidth = 9.dp, color = LifeOSPrimary, trackColor = LifeOSAccentLavender)
                }
            }
        }
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.fillMaxWidth()) {
                InsightStatCard("Tasks", "${stats.tasksCompleted}/${stats.tasksTotal}", Icons.Filled.TrendingUp, Modifier.weight(1f))
                InsightStatCard("Spend", "₹${"%.0f".format(stats.spend)}", Icons.Filled.AutoAwesome, Modifier.weight(1f))
                InsightStatCard("Diary", stats.diaryEntries.toString(), Icons.Filled.AutoAwesome, Modifier.weight(1f))
            }
        }
        ...
        item {
            LifeOSCard(Modifier.fillMaxWidth(), tint = MaterialTheme.colorScheme.surfaceContainerLow) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Icon(Icons.Filled.AutoAwesome, contentDescription = null, tint = LifeOSPrimary)
                    Column(Modifier.weight(1f).padding(start = 12.dp)) {
                        Text("Review your week", style = MaterialTheme.typography.titleMedium)
                        Text("LifeOS analyzes your local tasks, diary and spending without a cloud model.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                }
                Button(onClick = viewModel::generateLocalReview, enabled = !busy, modifier = Modifier.fillMaxWidth().padding(top = 14.dp)) {
        ...
        summary?.let { report ->
            item {
                LifeOSCard(Modifier.fillMaxWidth()) {
                    ...
                    Text(report, style = MaterialTheme.typography.bodyMedium, modifier = Modifier.padding(top = 8.dp))
                }
            }
        }
        item {
            LifeOSCard(Modifier.fillMaxWidth()) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Icon(Icons.Filled.Lock, contentDescription = null, tint = LifeOSPrimary)
...
@Composable
private fun InsightStatCard(label: String, value: String, icon: androidx.compose.ui.graphics.vector.ImageVector, modifier: Modifier = Modifier) {
    LifeOSCard(modifier) {
        Icon(icon, contentDescription = null, tint = LifeOSPrimary, modifier = Modifier.height(20.dp))
        Text(value, style = MaterialTheme.typography.titleMedium, modifier = Modifier.padding(top = 10.dp))
